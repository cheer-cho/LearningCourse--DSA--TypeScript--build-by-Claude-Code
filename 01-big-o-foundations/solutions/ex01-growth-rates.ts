...
export function growthRates(): Record<'A' | 'B' | 'C' | 'D' | 'E' | 'F' | 'G' | 'H', ComplexityClass> {
  // Pattern: read complexity straight off code shape — no algorithm to run,
  // just classification. Time: O(1), Space: O(1) (a fixed lookup table).
  return {
    A: 'O(1)', // one indexed read, no loop
    B: 'O(n)', // one pass over the array
    C: 'O(n^2)', // nested loop over the same array
    D: 'O(log n)', // halves n every step
    E: 'O(n)', // two SEQUENTIAL passes: O(n) + O(n) = O(n)
    F: 'O(n log n)', // comparison sort
    G: 'O(2^n)', // two recursive calls per step, each shrinking by 1
    H: 'O(n^2)', // n x n matrix has n^2 cells, visited once each
  }
}
